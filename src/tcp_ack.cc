...
void tcp_ack::action(const char* shost, int sport, const char* dhost, int dport) {
	srandom(time(NULL));
	...
	char buf[128];
	if (send_ack(sd, saddr, daddr, buf, sizeof(buf), sport, dport, seq) < 0) {
		goto exit;
	}
	while (true) {
		int ret = recv_rst(sd, saddr, daddr, buf, sizeof(buf), sport, dport);
		if (ret < 0) {
			goto exit;
		} else if (ret > 0) {
			break;
...
int tcp_ack::send_ack(int sd, const struct in_addr saddr, const struct in_addr daddr, char* buf, int len, u_int16_t sport, u_int16_t dport, u_int32_t seq) {
	memset(buf, 0, len);
...
int tcp_ack::recv_rst(int sd, const struct in_addr saddr, const struct in_addr daddr, char* buf, int len, u_int16_t sport, u_int16_t dport) {
	struct sockaddr_in sa;
	memset(&sa, 0, sizeof(sa));
	...
	if (ip->saddr != daddr.s_addr || dport != ntohs(tcp->source) || tcp->rst != 1) {
//		printf("recv_rst filtered - ip: packet_len=%d saddr=%s daddr=%s\n", ret, sip, dip);
		return 0;
	}
	
	printf("recv rst addr: dst=%s port=%d len=%d\n", inet_ntoa(sa.sin_addr), ntohs(sa.sin_port), salen);
	printf("recv rst iphdr: packet_len=%d saddr=%s daddr=%s\n", ret, sip, dip);
	printf("recv rst tcphdr: source=%d dest=%d seq=%u ack_seq=%u doff=%d fin=%d syn=%d rst=%d psh=%d ack=%d urg=%d window=%d check=%04x\n",
		ntohs(tcp->source), ntohs(tcp->dest), ntohl(tcp->seq), ntohl(tcp->ack_seq),
		tcp->doff, tcp->fin, tcp->syn, tcp->rst, tcp->psh, tcp->ack, tcp->urg, ntohs(tcp->window), tcp->check);
...
u_int16_t tcp_ack::check_sum(const char* buf, int len) { 
   u_int32_t sum = 0;
   while (len > 1) {
     sum += *(u_int16_t*)buf;
	 buf += 2;
